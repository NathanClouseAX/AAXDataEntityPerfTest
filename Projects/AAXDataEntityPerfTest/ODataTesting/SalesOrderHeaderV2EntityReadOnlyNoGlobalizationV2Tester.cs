...
namespace ODataTesting
{
    public class SalesOrderHeaderV2EntityReadOnlyNoGlobalizationTester : SalesOrderTester
    {
        ...
        public static string Entity = "SalesOrderHeaderV2EntityReadOnlyNoGlobalization";
        ...
        public static void runReads(Resources context, string filePath, TestType testType, TestWorkload testWorkload, string DataAreaId, string customerAccount, int count)
        {
            ...
            context.SalesOrderHeadersV2EntityReadOnlyNoGlobalization.Where(x => x.dataAreaId == DataAreaId && x.OrderingCustomerAccountNumber == customerAccount).Take(count).ToList();
        ...
        public static void runReadWithSelect(Resources context, string filePath, TestType testType, TestWorkload testWorkload, string SalesOrderNumber, string DataAreaId)
        {
            ...
            var SalesOrderHeadersV2EntityReadOnlyNoGlobalization = context.SalesOrderHeadersV2EntityReadOnlyNoGlobalization.Where(x => x.SalesOrderNumber == SalesOrderNumber && x.dataAreaId == DataAreaId)
        ...
        public static void runReadsWithSelect(Resources context, string filePath, TestType testType, TestWorkload testWorkload, string DataAreaId, string customerAccount, int count)
        {
            ...
            context.SalesOrderHeadersV2EntityReadOnlyNoGlobalization.Where(x => x.dataAreaId == DataAreaId && x.OrderingCustomerAccountNumber == customerAccount)
                                                        .Select(x => new { x.SalesOrderNumber, x.OrderingCustomerAccountNumber, x.InvoiceCustomerAccountNumber, x.SalesOrderStatus, x.dataAreaId })
                                                        .Take(count)
                                                        .ToList();
