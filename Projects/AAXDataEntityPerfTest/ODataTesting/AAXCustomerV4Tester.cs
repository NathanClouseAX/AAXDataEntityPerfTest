    ...
    public class AAXCustomerV4Tester : CustomerTester
    {
        ...
        public static void runOneRead(Resources context, string filePath, TestType testType, TestWorkload testWorkload, string CustomerAccount, string DataAreaId)
        {
            ...
            while (true)
            {
                ...
                {
                    tryCount++;
                    AAXCustomerV4 AAXCustomerV4 = context.AAXCustomersV4.Where(x => x.CustomerAccount == CustomerAccount && x.dataAreaId == DataAreaId).First();
